...
function findColumnLink($, headingText) {
  const heading = $("h6.wp-block-heading").filter((_, node) => $(node).text().trim() === headingText).first();
  return heading.closest(".wp-block-column").next(".wp-block-column").find("a").attr("href") ?? null;
}
